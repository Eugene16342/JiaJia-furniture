...
export default function payment_controller() {
  ...
  async function computed_total_price() {
    ...
    console.log("這是要發送請求的項目", select_items);
    try {
  ...
  async function get_user_info() {
    try {
      ...
      console.log("完整 API 返回數據：", data); // 確認 API 返回值
  ...
  async function create_order() {
    ...
    if (await order_confirm()) {
      try {
        ...
        console.log("訂單成立 單號為:", data.order_id);
        notification_method.success(
          `訂單提交成功！您的單號是：${data.order_id}`
        );
